...
public static class AppModuleExtensions {
	...
	public static IEnumerable<IAppModuleOptions> GetModulesOptions(this AppDomain domain, AppModuleLayerType? layer = null) {

		return domain.GetModulesAssemblies(layer).GetModulesOptions(layer);

	}

	public static IEnumerable<IAppModuleOptions> GetModulesOptions(this IEnumerable<Assembly> assemblies, AppModuleLayerType? layer = null) {

		return assemblies.SelectMany(static assembly => assembly.GetExportedTypes()).GetModulesOptions(layer);

	}

	public static IEnumerable<IAppModuleOptions> GetModulesOptions(this IEnumerable<Type> types, AppModuleLayerType? layer = null) {
		...
		foreach (var type in types) {
			...
			if (!TryExtractModuleAndLayer(type.Namespace, out _, out var layName)) continue;
			...
			if (!type.IsClass || type.IsAbstract || !typeof(IAppModuleOptions).IsAssignableFrom(type)) continue;
			...
			if (instance is IAppModuleOptions options) {
